...
def create_plugins(plugins):
    ...
    for it, plg in enumerate(watts_wb.plugins.plugin):
        ...
        if plg.module_dir is not None:
            nested_plugins['module_dir'] = str(
                plg.module_dir.value).strip('\"')
        ...
        if plg.output is not None:
            nested_plugins['output'] = str(plg.output.value)
...
def run_direct(watts_params, plugin):
    ...
    if plugin['code'].upper() == 'MOOSE':
        ...
        if 'exec_dir' in plugin:
            app_dir = Path(os.environ[plugin['exec_dir']])
        else:
            raise RuntimeError("Please provide path to MOOSE app directory.")
        ...
        # Insert path of directory OpenMC template
        # file then import the OpenMC model.
        if 'module_dir' in plugin:
            sys.path.insert(0, plugin['module_dir'])
        from openmc_template import build_openmc_model

        app_plugin = watts.PluginOpenMC(
            build_openmc_model, show_stderr=True)  # show only error
...
def run_iterate(watts_params, plugin, wf_level):
    ...
    while conv:
        ...
        # multiple values (especially for transient).
